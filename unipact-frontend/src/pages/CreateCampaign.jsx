...
const CreateCampaign = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-void)] p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        {/* Navigation back */}
        <button
          onClick={() => navigate('/company/dashboard')}
          className="text-xs text-[var(--text-gold)] uppercase flex items-center gap-2 mb-6 hover:underline"
        >
          <ArrowLeft size={14} /> Back to Dashboard
        </button>

        <div className="bg-[var(--bg-panel)] border border-[var(--border-tech)] p-8 relative shadow-2xl">
          {/* Header */}
          <div className="border-b border-[var(--border-tech)] pb-6 mb-8">
            <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-[var(--text-gold)] mb-1">
              <Sparkles size={14} /> Curated Talent Marketplace Order
            </div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white">Post a New Project Order</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              <label className="text-xs uppercase tracking-wider font-semibold text-[var(--text-gold)] block mb-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                  ...
                  className={`cursor-pointer p-4 rounded border transition-all flex items-start gap-3.5 ${
                    category === 'SOFTWARE_DEVELOPMENT'
                      ...
                      : 'border-[var(--border-tech)] bg-black/20 hover:border-slate-500'
                  }`}
                >
                  <Code2 className={`w-6 h-6 mt-0.5 ${category === 'SOFTWARE_DEVELOPMENT' ? 'text-[var(--text-gold)]' : 'text-slate-400'}`} />
                  <div>
                    <h3 className="text-sm font-bold text-white">Software Development</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Automation websites, CRM, ERP, HRMS, Landing Pages.</p>
                  </div>
                </div>
                ...
                >
                  <Video className={`w-6 h-6 mt-0.5 ${category === 'DIGITAL_MARKETING' ? 'text-cyan-400' : 'text-slate-400'}`} />
                  <div>
                    <h3 className="text-sm font-bold text-white">Digital Marketing</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Content campaigns, raw video editing, TikTok / Reels KPIs.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {category === 'SOFTWARE_DEVELOPMENT' ? (
              <div className="p-5 bg-black/30 border border-yellow-500/20 rounded space-y-4">
                <span className="text-xs font-bold text-[var(--text-gold)] uppercase tracking-wider block">
                  Software Development Specifications (REQ-3.3.1)
                </span>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-slate-300 block mb-1">Sub-Type (*)</label>
                    <select
                      value={softwareSubType}
                      onChange={(e) => setSoftwareSubType(e.target.value)}
                      className="w-full bg-black/50 border border-[var(--border-tech)] text-white p-2 text-xs rounded focus:border-[var(--text-gold)] focus:outline-none"
                    >
                  ...
                  <div>
                    <label className="text-xs text-slate-300 block mb-1">Required Skills / Tech Stack (*)</label>
                    <input
                      type="text"
                      value={techStack}
                ...
                <div>
                  <label className="text-xs text-slate-300 block mb-1">Project Outcome Description</label>
                  <textarea
                    rows={2}
              ...
              <div className="p-5 bg-black/30 border border-cyan-500/20 rounded space-y-4">
                <span className="text-xs font-bold text-cyan-400 uppercase tracking-wider block">
                  Digital Marketing Specifications (REQ-3.3.1)
                </span>
                <div>
                  <label className="text-xs text-slate-300 block mb-1">Campaign Objective (*)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs text-slate-300 block mb-2">Target Platforms & Channels</label>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-[var(--border-tech)] text-white p-2.5 text-sm rounded focus:border-[var(--text-gold)] focus:outline-none"
                />
              </div>

              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">Project Description (* Markdown)</label>
                <textarea
                  rows={4}
                  required
                  value={description}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Project Budget (MYR) (*)</label>
                  <input
                    type="number"
                    required
                    min={100}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Matching & Delivery Deadline</label>
                  <input
                    type="date"
                    value={deadline}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-semibold text-slate-300">Required Deliverables Checklist (*)</label>
                <button
                  type="button"
                ...
                {deliverables.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1 bg-black/40 border border-[var(--border-tech)] text-white p-2 text-xs rounded focus:border-[var(--text-gold)] focus:outline-none"
                    />
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-[var(--border-tech)] hover:border-[var(--text-gold)] rounded p-4 text-center cursor-pointer relative bg-black/20 transition-colors">
                <input
                  type="file"
                  onChange={(e) => e.target.files && setAssetFile(e.target.files[0])}
                  className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
                />
                <Upload className="mx-auto text-[var(--text-gold)] mb-1.5 w-6 h-6" />
                {assetFile ? (
                  ...
                  <div className="text-xs text-slate-400">
                    <span className="text-[var(--text-gold)] font-semibold">Attach raw project assets</span> (PDF brief, brand guidelines, or raw footage)
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Submit */}
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-[var(--border-tech)]">
              <button
                type="button"
                onClick={() => navigate('/company/dashboard')}
                className="px-4 py-2.5 rounded text-xs text-slate-400 hover:text-white border border-slate-700"
              >
                Cancel
              </button>
              <button
